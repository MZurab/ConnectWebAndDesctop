define(['jquery','v_category','m_view','m_app'], function ( $, V_CATEGORY, M_VIEW, M_APP) {
    ...
    function safeUpdateChatBlock (iNdata,chatType) {
        ...
        */
        ...
        var chatObject = V_CATEGORY.findChatBlock(iNdata);
        console.log('safeUpdateChatBlock chatObject',chatObject);
        ...
        console.log('safeUpdateChatBlock uuid',uuid);
                ...
                V_CATEGORY.setEffectsForChatList(chatObject);
		} 
        domChangeChatBlock (chatObject,iNdata);
    }
    _['safeUpdateChatBlock'] = safeUpdateChatBlock;
   
	function domChangeChatBlock (chatObject,iNdata) {
	    /*
	    ...
	    */
	    ...
	    // increase new msg count by $newMsgCount if it isset
	        if(  typeof(iNdata.newMsgCount) != 'undefined' )	V_CATEGORY.domPlusCountMessages(chatObject,iNdata.newMsgCount);
	    // change userName if userName isset
	        if(  typeof(iNdata.userName) != 'undefined' ) 		V_CATEGORY.domChangeUserNameInChatBlock(chatObject,iNdata.userName);
	    // chat last msg text and last msg time if isset lmsgText
	        if(  typeof(iNdata.lmsgText) != 'undefined' ) 		V_CATEGORY.domChangeLastMsgTextAndTimeInChatBlock(chatObject,iNdata);
	    // change icon if it isset
	        if(  typeof(iNdata.icon) != 'undefined' ) 			V_CATEGORY.domChangeIconInChatBlock(chatObject,iNdata.icon);
	    //addVirificateStatusToBlock
	        if( typeof(iNdata.verificate) != 'undefined' && iNdata.verificate == 1) V_CATEGORY.domAddVerificateStatusToChatBlock(chatObject);
	    ...
	    if( typeof(iNdata.liveStatus) != 'undefined' &&  typeof(iNdata.liveData) != 'undefined' &&  typeof(iNdata.liveType) != 'undefined' &&  typeof(iNdata.liveUser) != 'undefined' && USER.getMyId() != iNdata.liveUser){
	        switch(iNdata.liveType) {
	            case 1: // simple text chat
	                V_CATEGORY.domLiveSimpleTextAnimation(chatObject,iNdata);
	            break;
	        }

	        V_CATEGORY.startEffHideLiveInChatsList(chatObject + ' .liveBlocks');
	    }
	}
